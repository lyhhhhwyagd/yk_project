...
const paper = {
  ...
  actions: {
    getFullPaper: async({ commit }, paperId) => {
      console.log("getFullPaper",`get paper ${paperId}`);
      const res = await checkPaperAPI(paperId);
      if(res && res.data.success) {
        const paper = res.data.content
        console.log("getFullPaper",paper);
        // fill answers
        for(let question of paper.question1List) {
          console.log("getFullPaper question=", question)
          console.log("getFullPaper question.type=", question.type)
          if(question.type === 2) {
            question.answer = []
          } else {
            question.answer = ''
    ...
    getFullPaperStatistic: async({commit},paperId) =>{
       ...
       */
      console.log("**************************************************")
      axios.get(`http://localhost:8080/api/paper/${paperId}/reviewPaper`).then(res => {
      ...
      if(res && res.data.success) {
        ...
        console.log("here paperStatistic = res.data.content", paperStatistic)
        for(let question of paperStatistic.questionStatistics) {
          if(question.type === 3) {
            continue
          }
          const total = question.filledInNum
          console.log("question.optionStatistics",question.optionStatistics)
    ...
    restartCollection: async({ commit, getters }, date) => {
      ...
      console.log("修改之前restartCollection mp:", mp);
      let newPaperInfo = {
        ...mp,
      }
      if(date) {
