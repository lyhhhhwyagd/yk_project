...
//    where problemName like concat('%',#{problemName},'%')  and teacherId like concat('%',#{teacherId},'%')
    ...
    @Insert("insert into xm04_problem (problemname,problemdes,teacherid,a,b,c,d,ans,time) values (#{problemname},#{problemdes},#{teacherid},#{a},#{b},#{c},#{d},#{ans},#{time})")
...
//    IPage pageCC(IPage<Problem> page,@Param(Constants.WRAPPER ) Wrapper wrapper);
    ...
    @Delete("delete  from  xm04_problem where id=#{id}")
    void deleteById(Integer id);

    @Select("select * from xm04_problem ")
